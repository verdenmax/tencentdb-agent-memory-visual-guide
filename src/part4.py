...
<p>
同一段正文为什么有两个字段名：<span class="inline">L0MessageRecord</span> 是写入 JSONL 的<strong>原始落盘记录</strong>（字段名 <span class="inline">content</span>），而 <span class="inline">L0Record</span> 是为<strong>向量 / 搜索索引</strong>构造的投影记录，把同一段文本改名为 <span class="inline">messageText</span>。内容相同、用途不同，所以字段名不同。
</p>
...
<p>
Why the same text carries two field names: <span class="inline">L0MessageRecord</span> is the <strong>raw on-disk record</strong> written to JSONL (its field is <span class="inline">content</span>), while <span class="inline">L0Record</span> is the projection built for the <strong>vector / search index</strong>, which renames the same text to <span class="inline">messageText</span>. Same content, different purpose &mdash; hence the different field name.
</p>
...
<div class="cols">
  ...
  <div class="col"><h4>TCVDB backend</h4><p><span class="inline">src/core/store/tcvdb.ts</span> integrates a managed vector database. <span class="inline">createStoreBundle</span> wires embedding, store backend selection, and degradation behavior so the writer does not depend on backend details.</p></div>
</div>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">src/core/store/factory.ts</span>: <span class="inline">createStoreBundle</span> connects embedding, store backends, and backend selection/degradation behavior</li>
