...
只看本轮 prompt build 之后追加的消息；如果这个缓存不可用，再用 checkpoint 里的 <span class="inline">afterTimestamp</span> 游标过滤。
因此 Auto Capture 不是把整段历史反复写入 L0，而是把提交后新出现的消息增量写入。
</p>
...
<pre class="code">performAutoCapture(messages):
    checkpoint.captureAtomically(sessionKey, cursor, () =&gt; {
        filtered = recordConversation(messages, cursor)
        return maxTimestamp(filtered)
    })
    upsert_l0_metadata(filtered)
    maybe_embed_l0_in_background()
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">src/utils/checkpoint.ts</span>：<span class="inline">captureAtomically</span> 持锁读写 cursor 与计数</li>
...
recorded before prompt build and slices the raw messages after that position; if that cache is unavailable, it falls back to the checkpoint <span class="inline">afterTimestamp</span> cursor.
Auto Capture therefore does not keep rewriting the whole history into L0. It appends only the incremental messages that appeared after the committed turn.
</p>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">src/utils/checkpoint.ts</span>: <span class="inline">captureAtomically</span> locks cursor and count updates</li>
