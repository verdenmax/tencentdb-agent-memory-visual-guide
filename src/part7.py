...
<div class="timeline">
  ...
  <div class="tl"><div class="tm">model call</div><div class="td">如果入口抛出 token overflow，<span class="inline">isTokenOverflowError</span> 设置 <span class="inline">_forceEmergencyNext</span>，让下一次 L3 pass 走紧急压缩，而不是在错误处理里直接内联返回。</div></div>
...
<div class="card warn">
  ...
  <ul>
    ...
    <li>温和/激进阶段保护当前任务节点：<span class="inline">l3-helpers.ts</span> 的 current-task node 保护，避免把 active MMD 指向的证据刚好删掉。</li>
    <li>紧急阶段保护 MMD task-map 消息、最新用户消息、有效工具对和最小保留集，而不是继续做节点级 current-task 过滤。</li>
...
on_llm_input_l3(messages, snapshot, state):
    ...
    if snapshot.total_tokens &lt; mild_threshold and not force_emergency:
        return messages
...
# inside on_llm_input_l3 outer try/catch:
    ...
    if isTokenOverflowError(error):
        state._forceEmergencyNext = True  # consumed by the next L3 pass
    raise error</pre>
...
<div class="timeline">
  ...
  <div class="tl"><div class="tm">model call</div><div class="td">If the entry throws token overflow, <span class="inline">isTokenOverflowError</span> sets <span class="inline">_forceEmergencyNext</span> so the next L3 pass runs emergency compression, rather than returning inline emergency output from the error handler.</div></div>
...
<div class="card warn">
  ...
  <ul>
    ...
    <li>Protect current-task nodes during mild/aggressive stages: <span class="inline">l3-helpers.ts</span> current-task node protection prevents deleting evidence targeted by the active MMD.</li>
    <li>In emergency mode, protect MMD task-map messages, the latest user message, valid tool pairs, and the minimum keep set rather than continuing node-level current-task filtering.</li>
