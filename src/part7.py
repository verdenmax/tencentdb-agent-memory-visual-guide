...
再把已经确认 offload 的旧工具区域按确定性快路径替换掉，随后用精确 token 快照判断压缩档位：中等压力只跑温和替换，
...
<div class="card warn">
  ...
  <ul>
    ...
    <li>激进/紧急阶段保护结构边界：激进删除按旧 offloaded 区域前缀推进；紧急保留 MMD task-map 消息、最新用户消息、有效工具对和最小保留集，而不是继续做节点级 current-task 过滤。</li>
...
then deterministically re-applies replacements for already confirmed offloads, then uses precise token snapshots to choose the tier: medium pressure runs only mild replacement,
...
<div class="vflow">
  ...
  <div class="step"><div class="num">5</div><div class="sc"><h4>aggressive -&gt; mild -&gt; emergency</h4><p><span class="inline">aggressiveCompressUntilBelowThreshold</span> first deletes old offloaded regions and injects history MMD; mild cleanup then replaces the highest replaceability-score regions, and <span class="inline">emergencyCompress</span> runs only if still above the emergency threshold.</p><div class="mono">history MMD -&gt; high-score replacement -&gt; emergency trim</div></div></div>
</div>

<h2>Three compression tiers</h2>
<div class="cols">
  <div class="col"><h4>Aggressive: delete old offloaded regions</h4><p><b>Trigger</b>: token usage reaches the aggressive threshold, which is higher than the mild threshold.<br><b>Action</b>: <span class="inline">aggressiveCompressUntilBelowThreshold</span> first removes older offloaded message blocks and injects history MMD.<br><b>Evidence recovery</b>: go from history MMD nodes to JSONL, then to refs.</p></div>
...
<div class="card warn">
  ...
  <ul>
    ...
    <li>Protect structural boundaries during aggressive/emergency stages: aggressive deletion advances by old offloaded-region prefixes; emergency keeps MMD task-map messages, the latest user message, valid tool pairs, and the minimum keep set rather than continuing node-level current-task filtering.</li>
