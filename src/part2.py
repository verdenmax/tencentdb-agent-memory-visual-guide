...
<div class="cols">
  ...
  <div class="col"><h4>TCVDB backend</h4><p>当数据规模、共享部署或远程向量检索成为真实需求，再配置 <span class="inline">tcvdb</span>。不要只填 <span class="inline">tcvdb</span> 字段：还必须在 <span class="inline">memory-tencentdb.config</span> 的顶层设置 <span class="inline">storeBackend: "tcvdb"</span>，store bundle 才会走远程后端。</p></div>
</div>

<p>TCVDB 路径需要两个部分同时出现在 <span class="inline">memory-tencentdb.config</span> 中：顶层后端选择和 <span class="inline">tcvdb</span> 连接占位符。示例只使用假 URL 与环境变量名：</p>
<pre class="code">{
  "memory-tencentdb": {
    "config": {
      "storeBackend": "tcvdb",
...
<h2>从配置到行为的路径</h2>
<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>openclaw.plugin.json 暴露入口</h4><p>插件元数据、activation / config schema 和 tool contracts 告诉 OpenClaw 这个包的入口与配置面；运行时 hook 注册在 <span class="inline">index.ts</span>。</p></div></div>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">openclaw.plugin.json</span>：插件元数据、activation / config schema 和 tool contracts；运行时 hook 注册在 <span class="inline">index.ts</span>。</li>
...
<div class="cols">
  ...
  <div class="col"><h4>TCVDB backend</h4><p>Configure <span class="inline">tcvdb</span> only when scale, shared deployment, or remote vector retrieval is a real requirement. Do not fill only the <span class="inline">tcvdb</span> object: set <span class="inline">storeBackend: "tcvdb"</span> at the top level of <span class="inline">memory-tencentdb.config</span> so the store bundle uses the remote backend.</p></div>
</div>

<p>The TCVDB path needs both pieces inside <span class="inline">memory-tencentdb.config</span>: top-level backend selection and placeholder <span class="inline">tcvdb</span> connection fields. Use fake URLs and environment variable names in examples:</p>
<pre class="code">{
  "memory-tencentdb": {
    "config": {
      "storeBackend": "tcvdb",
...
<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>openclaw.plugin.json exposes the entry</h4><p>Plugin metadata, activation/config schema, and tool contracts tell OpenClaw the package entry and configuration surface; runtime hook registration lives in <span class="inline">index.ts</span>.</p></div></div>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">openclaw.plugin.json</span>: plugin metadata, activation/config schema, and tool contracts; runtime hook registration lives in <span class="inline">index.ts</span>.</li>
