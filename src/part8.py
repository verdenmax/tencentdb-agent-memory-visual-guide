...
<span class="inline">src/gateway/config.ts</span> 解析监听地址、CORS、鉴权、payload 限制和超时等安全开关。通过这些守卫后，Gateway 暴露的 memory 端点才会
...
<div class="flow">
  ...
  <div class="arrow">-&gt;</div>
  <div class="node"><div class="nt">Gateway HTTP route</div><div class="nd"><span class="inline">server.ts</span> 创建 HTTP server，注册 health、status 与 memory routes。</div></div>
  <div class="arrow">-&gt;</div>
  <div class="node"><div class="nt">config/security guard</div><div class="nd"><span class="inline">config.ts</span> 解析 host、port、origin、auth、body limit、timeout。</div></div>
  <div class="arrow">-&gt;</div>
...
<div class="vflow">
  ...
  <div class="step"><div class="num">2</div><div class="sc"><h4>public health/status</h4><p><span class="inline">/health</span> 用于 liveness；<span class="inline">/status</span> 用于确认功能是否启用。它们只能返回安全状态，不应泄露 token、完整路径或云账号信息。</p><div class="mono">GET /health -&gt; ok</div></div></div>
  <div class="step"><div class="num">3</div><div class="sc"><h4>validate request</h4><p>memory routes 在委托前检查方法、路径、content-type、payload 大小、JSON schema 形状、timeout 和可选 auth token。</p><div class="mono">reject early before TdaiCore</div></div></div>
...
<div class="cols">
  ...
  <div class="col"><h4>Hermes / Gateway external HTTP path</h4><p>Hermes provider 或直接调用方通过 HTTP 调用 Gateway。网络入口必须先处理绑定地址、CORS、鉴权、body limit、超时和日志脱敏，然后才委托 standalone adapter。主要风险是暴露面、滥用、跨源访问和错误日志泄密。</p></div>
...
<h2>config.ts：安全开关要在启动时讲清楚</h2>
<table class="t">
  <tr><th>Knob</th><th>Default idea</th><th>Risk if loose</th><th>Safe example</th></tr>
  ...
  <tr><td class="mono">allowed origins</td><td>deny by default or narrow allow-list</td><td>浏览器跨源页面可误用本机 Gateway。</td><td><span class="inline">https://APP_HOST_PLACEHOLDER</span></td></tr>
  <tr><td class="mono">auth token</td><td>required outside trusted loopback</td><td>未授权调用可读取、写入或搜索记忆。</td><td><span class="inline">Authorization: Bearer TOKEN_PLACEHOLDER</span></td></tr>
  <tr><td class="mono">payload size</td><td>small bounded JSON</td><td>大 body 占内存、拖慢解析，或把工具日志误塞进 HTTP。</td><td><span class="inline">MAX_BODY_BYTES=1048576</span></td></tr>
  <tr><td class="mono">timeout</td><td>bounded per request</td><td>慢请求耗尽 worker 或让用户误判为 Gateway 挂死。</td><td><span class="inline">REQUEST_TIMEOUT_MS=30000</span></td></tr>
...
<div class="cellgroup">
  ...
  <div class="cg-cap"><b>gateway side</b>：Gateway 用自己的 env/config 解析 listen、CORS、auth、size 与 timeout，不从页面复制真实密钥。</div>
  <div class="cells"><span class="cell">TDAI_GATEWAY_HOST</span><span class="cell">TDAI_GATEWAY_PORT</span><span class="cell">TDAI_GATEWAY_API_KEY</span><span class="cell">TDAI_GATEWAY_ALLOWED_ORIGINS</span></div>
</div>
...
on_request(req):
    ...
    body = read_json_body(req, max_bytes=config.max_body_bytes)
    validate_memory_payload(route.name, body)

    with timeout(config.request_timeout_ms):
        context = standalone_host_adapter.from_http(req, body)
...
<div class="timeline">
  ...
  <div class="tl"><div class="tm">before enable</div><div class="td">设置 allowed origins、auth token、payload size 和 request timeout。health/status 可以轻量公开，但 memory routes 必须受保护。</div></div>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">src/gateway/server.ts</span>：HTTP server 创建、路由注册、health/status 端点、请求校验和向核心委托。</li>
    <li><span class="inline">src/gateway/config.ts</span>：env/config 解析、listen host/port、安全开关、auth token 与默认值。</li>
...
<div class="card key">
  ...
  <span class="inline">server.ts</span> 的路由与校验、<span class="inline">config.ts</span> 的安全开关，再由 standalone host adapter 收敛到 <span class="inline">TdaiCore</span>。
  安全默认值应偏向 loopback、窄 CORS、占位 token、小 payload、短 timeout、平台限流和日志脱敏；文档与示例只能使用占位凭据和占位端点。
</div>
""",
    "en": r"""
<p class="lead" style="font-size:1.06rem;color:var(--muted);margin-top:-.6rem">
...
<span class="inline">src/gateway/config.ts</span> resolves listen address, CORS, authentication, payload limits, timeouts, and other security switches. Only after those guards does the Gateway memory route
...
<div class="flow">
  ...
  <div class="arrow">-&gt;</div>
  <div class="node"><div class="nt">Gateway HTTP route</div><div class="nd"><span class="inline">server.ts</span> creates the HTTP server and registers health, status, and memory routes.</div></div>
  <div class="arrow">-&gt;</div>
  <div class="node"><div class="nt">config/security guard</div><div class="nd"><span class="inline">config.ts</span> resolves host, port, origin, auth, body limit, and timeout.</div></div>
  <div class="arrow">-&gt;</div>
...
<div class="vflow">
  ...
  <div class="step"><div class="num">2</div><div class="sc"><h4>public health/status</h4><p><span class="inline">/health</span> serves liveness; <span class="inline">/status</span> confirms feature state. They should return safe status only, never tokens, full paths, or cloud account data.</p><div class="mono">GET /health -&gt; ok</div></div></div>
  <div class="step"><div class="num">3</div><div class="sc"><h4>validate request</h4><p>Memory routes check method, path, content-type, payload size, JSON schema shape, timeout, and optional auth token before delegation.</p><div class="mono">reject early before TdaiCore</div></div></div>
...
<div class="cols">
  ...
  <div class="col"><h4>Hermes / Gateway external HTTP path</h4><p>A Hermes provider or direct caller invokes the Gateway over HTTP. The network entry must handle bind address, CORS, auth, body limit, timeout, and redacted logging before the standalone adapter. The main risks are exposure, abuse, cross-origin access, and secret leakage in error logs.</p></div>
...
<h2>config.ts: security switches should be explicit at startup</h2>
<table class="t">
  <tr><th>Knob</th><th>Default idea</th><th>Risk if loose</th><th>Safe example</th></tr>
  ...
  <tr><td class="mono">allowed origins</td><td>deny by default or narrow allow-list</td><td>A browser page from another origin may misuse a local Gateway.</td><td><span class="inline">https://APP_HOST_PLACEHOLDER</span></td></tr>
  <tr><td class="mono">auth token</td><td>required outside trusted loopback</td><td>Unauthorized callers may read, write, or search memory.</td><td><span class="inline">Authorization: Bearer TOKEN_PLACEHOLDER</span></td></tr>
  <tr><td class="mono">payload size</td><td>small bounded JSON</td><td>Large bodies consume memory, slow parsing, or stuff tool logs into HTTP.</td><td><span class="inline">MAX_BODY_BYTES=1048576</span></td></tr>
  <tr><td class="mono">timeout</td><td>bounded per request</td><td>Slow requests exhaust workers or make users think Gateway is hung.</td><td><span class="inline">REQUEST_TIMEOUT_MS=30000</span></td></tr>
...
<div class="cellgroup">
  ...
  <div class="cells"><span class="cell hl">provider key</span><span class="sep">-&gt;</span><span class="cell">GATEWAY_URL_PLACEHOLDER</span><span class="sep">-&gt;</span><span class="cell q">TOKEN_PLACEHOLDER</span></div>
  <div class="cg-cap"><b>gateway side</b>: Gateway resolves listen, CORS, auth, size, and timeout from its own env/config; never copy real secrets from a page.</div>
  <div class="cells"><span class="cell">TDAI_GATEWAY_HOST</span><span class="cell">TDAI_GATEWAY_PORT</span><span class="cell">TDAI_GATEWAY_API_KEY</span><span class="cell">TDAI_GATEWAY_ALLOWED_ORIGINS</span></div>
</div>
...
<div class="timeline">
  ...
  <div class="tl"><div class="tm">before enable</div><div class="td">Set allowed origins, auth token, payload size, and request timeout. Health/status can be lightweight, but memory routes must be protected.</div></div>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">src/gateway/server.ts</span>: HTTP server creation, route registration, health/status endpoints, request validation, and delegation to core.</li>
    <li><span class="inline">src/gateway/config.ts</span>: env/config resolution, listen host/port, security switches, auth token, and defaults.</li>
...
<div class="card key">
  ...
  <span class="inline">server.ts</span> routing and validation plus <span class="inline">config.ts</span> security switches before the standalone host adapter converges on <span class="inline">TdaiCore</span>.
  Safe defaults should favor loopback, narrow CORS, placeholder tokens, small payloads, short timeouts, platform rate limits, and redacted logs; docs and examples should use only placeholder credentials and placeholder endpoints.
</div>
""",
}
