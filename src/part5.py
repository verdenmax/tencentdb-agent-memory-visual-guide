...
<div class="cols">
  ...
  <div class="col"><h4>L2 scene responsibilities</h4><p>人类可读、情境化、可导航。一个 scene block 聚合多条相关 L1，维护活动摘要、时间线、证据链接、热度与更新历史。</p></div>
</div>

<h2>Scene Block 保存什么</h2>
<table class="t">
  <tr><th>字段</th><th>作用</th></tr>
  <tr><td class="mono">filename</td><td>场景块的稳定文件名，方便增量更新、导航和 persona 生成器读取。</td></tr>
  ...
  <tr><td class="mono">evidence</td><td>保留 L1 / L0 证据线索，让摘要可追溯，而不是变成无来源结论。</td></tr>
</table>

<p>
这些字段共同解决一个核心问题：L2 保存“这批事实为什么有关联”。它不会替代 L1 搜索，也不会删除 L0 原文；
它把多条 L1 的关系、顺序和证据组织成可读块。这样 recall 可以先注入相关场景摘要，需要核实时再沿 evidence 回到 L1 和 L0。
</p>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">src/core/prompts/scene-extraction.ts</span>：scene block prompt contract 要求模型输出可解析、带证据、可更新的场景块，而不是自由散文。</li>
    <li><span class="inline">src/core/scene/scene-format.ts</span>：<span class="inline">parseSceneBlock</span> 把文本块解析回结构化字段，保证后续流程能读取 metadata、summary 与 evidence。</li>
...
<div class="cols">
  ...
  <div class="col"><h4>L2 scene responsibilities</h4><p>Human-readable, contextual, navigable. A scene block clusters related L1 records and maintains activity summary, timeline, evidence links, heat, and update history.</p></div>
</div>

<h2>What a Scene Block preserves</h2>
<table class="t">
  <tr><th>Field</th><th>Purpose</th></tr>
  <tr><td class="mono">filename</td><td>Stable scene-block filename for incremental updates, navigation, and persona-generator reads.</td></tr>
  ...
  <tr><td class="mono">evidence</td><td>Links back to L1 / L0 evidence so summaries remain traceable instead of source-free claims.</td></tr>
</table>

<p>
Together, these fields answer the core L2 question: why are these facts related? L2 does not replace L1 search or delete raw L0 text.
It organizes relationships, order, and evidence across many L1 records into a readable block. Recall can inject the relevant scene summary first,
then drill through evidence back to L1 and L0 when verification is needed.
</p>
...
<div class="card detail">
  ...
  <ul>
    ...
    <li><span class="inline">src/core/prompts/scene-extraction.ts</span>: the scene block prompt contract asks the model for parseable, evidence-linked, updatable scene blocks rather than free-form prose.</li>
    <li><span class="inline">src/core/scene/scene-format.ts</span>: <span class="inline">parseSceneBlock</span> parses text blocks back into structured fields so downstream code can read metadata, summary, and evidence.</li>
