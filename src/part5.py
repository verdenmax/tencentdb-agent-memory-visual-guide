...
<div class="vflow">
  ...
  <div class="step"><div class="num">5</div><div class="sc"><h4>工程后处理</h4><p>运行结束后删除软删除文件（内容为 <span class="inline">[DELETED]</span> 标记，或只剩 META 头、没有正文的空场景）、规范化文件名、同步索引，并解析 persona update signals。</p><div class="mono">cleanup -&gt; normalize filenames -&gt; sync index -&gt; parse persona signals</div></div></div>
</div>

<h2>可见与隐藏的文件边界</h2>
<div class="layers">
  <div class="layer l-core"><div class="lh"><span class="badge">VISIBLE</span><span class="name">scene_blocks/</span></div><div class="ld">LLM 的文件工具（read/write/edit）只能在这里读写 Markdown scene files：可创建、更新，或把内容写成 <span class="inline">[DELETED]</span> 标记做软删除；它产生的文件名随后由工程代码统一规范化。</div></div>
...
注意模型没有 exec 或删除工具，软删除只能把文件内容写成 <span class="inline">[DELETED]</span>（写空内容会被 write 工具拒绝），真正的 <span class="inline">unlink</span> 由后处理完成。
...
<div class="vflow">
  ...
  <div class="step"><div class="num">5</div><div class="sc"><h4>Post-process in code</h4><p>After the run, code removes soft-deleted files (content is the <span class="inline">[DELETED]</span> marker, or only a META header with no body), normalizes filenames, syncs the index, and parses persona update signals.</p><div class="mono">cleanup -&gt; normalize filenames -&gt; sync index -&gt; parse persona signals</div></div></div>
</div>

<h2>Visible and hidden file boundary</h2>
<div class="layers">
  <div class="layer l-core"><div class="lh"><span class="badge">VISIBLE</span><span class="name">scene_blocks/</span></div><div class="ld">The LLM file tools (read/write/edit) can read/write Markdown scene files only here: create, update, or write the <span class="inline">[DELETED]</span> marker as content to soft-delete; the filenames it produces are later normalized by engineering code.</div></div>
...
Note that the model has no exec or delete tool, so soft-delete just writes <span class="inline">[DELETED]</span> as the file content (empty content is rejected by the write tool); the real <span class="inline">unlink</span> happens in post-processing.
...
on_l2_success():
    ...
    if destroyed:
        return
    arm_l2_max_interval_timer()        # periodic poll guarantee
    if l3Running:
        l3Pending = true
    else:
        enqueue_global_l3()
